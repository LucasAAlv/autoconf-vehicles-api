...
return [
    ...
    'title' => 'Documentação da API Autoconf Vehicles',

    // A short description of your API. Will be included in the docs webpage, Postman collection and OpenAPI spec.
    'description' => 'API para um SaaS multiusuário de gestão de veículos: autenticação, cadastro de veículos e gestão de imagens com capa única.',
    ...
    'routes' => [
        [
            ...
            'exclude' => [
                // Leftover default Laravel scaffold route (`GET /api/user`, an
                // inline closure returning the authenticated user). It isn't
                // part of this project's documented domain (auth/vehicles/
                // images), so it's excluded rather than documented.
                'GET /api/user',
            ],
        ],
    ],
    ...
    //
    // This project uses Sanctum's SPA (cookie-based) auth, not a bearer/header/query
    // token: there is no static credential for Scribe to inject into example requests
    // or response calls, so `enabled` stays `false` here. The actual auth flow (log in
    // via `POST /auth/login` to get a session cookie, then let `try_it_out.use_csrf`
    // above fetch and attach the CSRF token automatically) is documented in `intro_text`
    // instead, and each protected endpoint is still marked with the `#[Authenticated]`
    // attribute so the docs UI shows the right badge and a `401` example is included.
    'auth' => [
    ...
    'strategies' => [
        ...
        // `ResponseCalls` (which would fire real HTTP requests at `GET *` routes to
        // capture live responses) is deliberately removed: every endpoint below is
        // annotated with explicit `#[Response]`/`#[ResponseFromApiResource]` examples
        // instead, and all `vehicles`/`auth` routes sit behind `auth:sanctum` — a real
        // response call would need a genuine authenticated session and would hit
        // Postgres — neither of which belongs in a docs-generation step.
